function plot_all2(sol1,sol2,sol3,ip)

% Extract freezing data
time1 = sol1.t; Temp1 = sol1.T; Tb1 = sol1.Tg;

% Extract primary drying data
time2 = sol2.t; Temp2 = sol2.T; Tp2 = mean(Temp2,2); Tb2 = sol2.Tb;

% Extract secondary drying data
time3 = sol3.t; Temp3 = sol3.T; Tp3 = mean(Temp3,2); Tb3 = sol3.Tb; cw = mean(sol3.cw,2);

% Combine
time = [time1;time2;time3];
S = [sol1.S;sol2.S;sol3.S];
Temp = [Temp1;Tp2;Tp3];
Tb = [Tb2;Tb3];
% cw = [sol1.cw;sol2.cw;cw];
P = [sol1.P;sol2.P;sol3.P];

% Mass of ice
mi1 = sol1.mi;
mi2 = cal_mi(sol2.S,mi1(end),ip.H2);
mi3 = zeros(length(time3),1);
mi = [mi1;mi2;mi3];

tiledlayout(5,1,"TileSpacing","tight","Padding","compact")
nexttile(1)
% annotation('doublearrow',[.08 .269],[.97 .97])
% annotation('doublearrow',[.275 .657],[.97 .97])
% annotation('doublearrow',[.663 .94],[.97 .97])

plot(time1/3600,Temp1,'-','Color',[0 0 1, 1],'linewidth',2); hold on
plot(time2/3600,Tp2,'-','Color',[0 0.7 0.17, 1],'linewidth',2); hold on
plot(time3/3600,Tp3,'-','Color',[1 0.4 0, 1],'linewidth',2); hold on
xline(time1(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time2(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time3(end)/3600,'--','linewidth',1,'HandleVisibility','off')
ylim([220 300]) 
ylabel('Product temperature (K)')
% h = legend('location','northwest'); h.ItemTokenSize(1) = 10;
graphics_setup('wide_all2')
text(.02,.2,'a','Units','normalized','FontSize', 10,'fontweight', 'bold');

nexttile(2)
plot(time1/3600,mi1,'Color',[0 0 1, 1],'linewidth',2); hold on
plot(time2/3600,mi2,'Color',[0 0.7 0.17, 1],'linewidth',2); hold on
plot(time3/3600,mi3,'Color',[1 0.4 0, 1],'linewidth',2); hold on
xline(time1(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time2(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time3(end)/3600,'--','linewidth',1,'HandleVisibility','off')
% xlim([0 round(time3(end),-1)])
% ylim([min(Tb1)-10 round(max(Temp,[],'all')+10,-1)]) 
ylabel('Mass of ice (kg)')
% h = legend('location','northwest'); h.ItemTokenSize(1) = 10;
graphics_setup('wide_all2')
text(.02,.2,'b','Units','normalized','FontSize', 10,'fontweight', 'bold');

nexttile(3)
plot(time1/3600,sol1.cw,'Color',[0 0 1, 1],'linewidth',2); hold on
plot(time2/3600,sol2.cw,'Color',[0 0.7 0.17, 1],'linewidth',2); hold on
plot(time3/3600,cw,'Color',[1 0.4 0, 1],'linewidth',2); hold on
xline(time1(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time2(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time3(end)/3600,'--','linewidth',1,'HandleVisibility','off')
% xlim([0 round(time3(end),-1)])
% ylim([min(Tb1)-10 round(max(Temp,[],'all')+10,-1)]) 
ylabel([{'Concentration'};{'(kg water/kg solid)'}])
% h = legend('location','northwest'); h.ItemTokenSize(1) = 10;
graphics_setup('wide_all2')
text(.02,.2,'c','Units','normalized','FontSize', 10,'fontweight', 'bold');

nexttile(4)
semilogy(time1/3600,sol1.P,'Color',[0 0 1, 1],'linewidth',2); hold on
semilogy([time2(1)/3600; time2/3600],[sol1.P(end);sol2.P],'Color',[0 0.7 0.17, 1],'linewidth',2); hold on
semilogy(time3/3600,sol3.P,'Color',[1 0.4 0, 1],'linewidth',2); hold on
xline(time1(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time2(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time3(end)/3600,'--','linewidth',1,'HandleVisibility','off')
% xlim([0 round(time3(end),-1)])
ylim([0 1e6])
% ylim([min(Tb1)-10 round(max(Temp,[],'all')+10,-1)]) 
ylabel('Operating pressure (Pa)');
% h = legend('location','northwest'); h.ItemTokenSize(1) = 10;
graphics_setup('wide_all2')
text(.02,.2,'d','Units','normalized','FontSize', 10,'fontweight', 'bold');

nexttile(5)
plot(time1/3600,Tb1,'-','Color',[0 0 1, 1],'linewidth',2,'displayname','Cold gas'); hold on
plot(time2/3600,Tb2,'-','Color',[0 0.7 0.17, 1],'linewidth',2,'displayname','Heating shelf'); hold on
plot(time3/3600,Tb3,'-','Color',[1 0.4 0, 1],'linewidth',2,'HandleVisibility','off')
xline(time1(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time2(end)/3600,'--','linewidth',1,'HandleVisibility','off')
xline(time3(end)/3600,'--','linewidth',1,'HandleVisibility','off')
ylim([min(Tb1)-10 round(max(Temp,[],'all')+20,-1)]) 
xlabel('Time (h)'); ylabel('Operating temperature (K)')
% h = legend('location','best'); h.ItemTokenSize(1) = 10;
graphics_setup('wide_all2')
text(.02,.2,'e','Units','normalized','FontSize', 10,'fontweight', 'bold');

end